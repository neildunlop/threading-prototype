...
public class FlightMessageAssociator implements Callable {
	...
	public FlightMessageAssociator(ConcurrentMap<String, Long> flightList, BlockingQueue<FlightAltitudeUpdate> inboundQueue, BlockingQueue<FlightAltitudeUpdate> outboundQueue) {
		super();
	...
	public Object call() throws Exception {
		...
		System.out.println("Doing association on Flight '" + message.getIcao()
				+ "' at " + new DateTime(currentTime).toString());
		
		//flight association can be tricky in production.
		//Thread.sleep(100);
		...
		//(These would be axon commands in production and we would block the sending
		//of the event to CEP until we get a response from the Future we send to Axon).
		Long putResult = FlightMessageAssociator.flightList
				.putIfAbsent(message.getIcao(), currentTime);
		if (putResult == null) {
			System.out.println("Creating Flight '" + message.getIcao()
					+ "' at " + new DateTime(currentTime).toString());
		} else {
			System.out.println("Updating Flight '" + message.getIcao()
					+ "' at " + new DateTime(currentTime).toString());
		}

		//TODO: Comment back in.. removed just to see if we can get a full flow of messages
